...
pub const SimConfig = struct {
    ...
    user_policy: Categorical(Precision, entities.Action),
    user_inter_action: Exponential(Precision), // time between a user two actions
    // to init posts
    warmup_post_inter_creation: Uniform(Precision), // time of the post created in the simulation
    // delays on posts transmissions
    propagation_delay: Constant(Precision), // time between an action over a post and showing up followers timeline
    interaction_delay: Constant(Precision), // time between
    // session configuration
    offline_startup_ratio: Precision, // which proportion of the users start on vacation
    creation_delay: Constant(Precision),
    trace_to_file: bool,
    ...
    pub fn calibrate(gpa: Allocator) !SimConfig {
        return SimConfig{
            ...
            .duration = 5000,
            .warmup_time = 1000,
            .user_policy = try Categorical(Precision, entities.Action).init(gpa, &.{ 0.80, 0.188, 0.012 }, &.{ .ignore, .like, .repost }),
            .user_inter_action = Exponential(Precision).initMean(3.0),
            .warmup_post_inter_creation = Uniform(Precision).init(0.0, 1000.0, Interval.cc),
            .propagation_delay = Constant(Precision).init(1.0),
            .interaction_delay = Constant(Precision).init(1.0),
            .offline_startup_ratio = 0.5,
            .creation_delay = Constant(Precision).init(1.0),
            .trace_to_file = @import("build").trace_to_file,
        };
    }
